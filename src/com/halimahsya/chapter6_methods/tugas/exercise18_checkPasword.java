...
public class exercise18_checkPasword {
    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);
        System.out.println(" masukan pasword : ");
        String password = input.nextLine();

        System.out.println((isValidPassword(password) ? "Valid" : "tidak valid") + "pasword");
    }

    public static boolean isValidPassword(String password) {
        final int LENGTH_OF_VALID_PASSWORD = 8;
        final int MININUM_NUMBER_OF_DIGITS = 2;
        boolean validPassword =
                isLengthValid(password, LENGTH_OF_VALID_PASSWORD) &&
                        isOnlyLettersAndDigits(password) &&
                        hasNDigits(password, MININUM_NUMBER_OF_DIGITS);
